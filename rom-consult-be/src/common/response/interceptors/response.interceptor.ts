      ...
      map((responseBody) => {
        ...
        const data = classSerialization
          ? isPaginatedResponse &&
            ...
            Array.isArray((responseBody as { items?: unknown }).items) &&
            (responseBody as { metadata?: unknown }).metadata !== undefined &&
            typeof (responseBody as { metadata?: unknown }).metadata ===
              "object"
            ? {
                items: (responseBody as { items: unknown[] }).items.map(
                  (item) =>
                    plainToInstance(
                      classSerialization,
                      normalizeDecimalsForResponseSerialization(item),
                      {
            ...
            : plainToInstance(
                classSerialization,
                normalizeDecimalsForResponseSerialization(responseBody),
                {
                  excludeExtraneousValues: true,
                },
              )
