...
export class HelperPrismaQueryBuilderService implements IHelperPrismaQueryBuilderService {
  ...
  private buildWhereClause(
    ...
    // Basic filters
    if (options.filters) {
      const basicFilters = this.buildBasicFilters(options.filters, config);
    ...
    if (options.searchQuery && options.searchFields?.length) {
      const searchConditions = this.buildSearchConditions(options, config);
      if (searchConditions) {
    ...
    if (options.rangeFilters?.length) {
      const rangeConditions = this.buildRangeFilters(options.rangeFilters);
  ...
  private buildSearchConditions(
    ...
    const { searchQuery, searchFields, searchMode = "insensitive" } = options;
