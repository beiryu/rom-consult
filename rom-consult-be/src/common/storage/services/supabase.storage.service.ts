...
export class SupabaseStorageService implements IStorageService {
  ...
  private resolveBucketName(kind: SupabaseStorageBucketKind): string {
    if (kind === "publicAssets") {
      if (!this.publicAssetsBucket) {
        throw new Error("SUPABASE_STORAGE_BUCKET_PUBLIC_ASSETS is not set");
      }
  ...
  async uploadObject(
    ...
    try {
      ...
      this.logger.info({ key, contentType }, "Object uploaded to storage");
    } catch (error) {
      this.logger.error(`Failed to upload object to storage: ${error.message}`);
      throw error;
    }
  }
