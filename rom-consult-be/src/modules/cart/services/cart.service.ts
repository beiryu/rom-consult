...
export class CartService implements ICartService {
  ...
  async addItem(
    ...
    try {
      ...
      if (existingItem) {
        ...
        await this.validateProductLine(data.productId, newQuantity);
  ...
  async updateItem(
    ...
    try {
      ...
      await this.validateProductLine(cartItem.productId, data.quantity);
   ...
   */
  async syncCart(userId: string, data: CartSyncDto): Promise<CartResponseDto> {
    try {
      const cart = await this.getOrCreateCart(userId);
