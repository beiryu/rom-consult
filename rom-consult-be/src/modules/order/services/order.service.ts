...
export class OrderService implements IOrderService {
  ...
  private async validateCartForOrder(userId: string): Promise<void> {
    ...
    if (!cart || !cart.items || cart.items.length === 0) {
      throw new HttpException("order.error.cartEmpty", HttpStatus.BAD_REQUEST);
    }
  ...
  async createOrder(
    ...
    try {
      ...
      const { totalAmount, currency } = calculateLineItemsTotals(cart.items);
  ...
  async getOrderHistory(
    ...
    try {
      ...
      const result = await this.paginationService.paginate<OrderResponseDto>(
