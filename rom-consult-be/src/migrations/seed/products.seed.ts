...
export class ProductsSeedService {
  ...
  @Command({
    ...
    describe:
      "Seed product categories and products matching browse-services page",
  })
  async seed(): Promise<void> {
    ...
    try {
      ...
      this.logger.info(`Ensured ${Object.keys(categories).length} categories`);
  ...
  > {
    ...
    for (const row of CATEGORY_SEEDS) {
      const existing = await this.databaseService.productCategory.findUnique({
        where: { slug: row.slug },
      });
      ...
      if (existing) {
        ...
        const created = await this.databaseService.productCategory.create({
          data: {
            name: row.name,
            slug: row.slug,
            icon: row.icon,
